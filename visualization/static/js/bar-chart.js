function countryCountBarChart(barChartSelector, colors) {
  ...
  function updateBarChart() {
    ...
    dataSet = pickTop10(dataSet);
    updateScales(dataSet);
    updateBars(dataSet);
    updateBarLabels(dataSet);
  }

  function pickTop10(dataSet) {
  ...
  function createBarChartDataSet() {
    var dataSet = [];
    Object.keys(countryCount).forEach(function(lang) {
      dataSet.push({
        lang: lang,
        count: countryCount[lang]
      });
    });
    return dataSet;
  }
  ...
  function addGeoData(data) {
    updateCountryCount(data.lang);
  }

  function updateCountryCount(lang) {
    var count = 0;
    if (countryCount[lang]) {
      count = countryCount[lang];
    }
    count += 1;
    countryCount[lang] = count;
  }
